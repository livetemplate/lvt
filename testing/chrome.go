...
func fetchCSS() ([]byte, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("CSS CDN fetch failed (no cache available): %w", err)
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("CSS CDN returned status %d for %s (no cache available)", resp.StatusCode, cdnURL)
	}
